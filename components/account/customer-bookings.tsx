...
function EditForm({ catalog, serviceId, employeeId, date, time, slots, slotsLoading, saving, price, onService, onEmployee, onDate, onTime, onCancel, onSubmit }: { catalog: Catalog | null; serviceId: string; employeeId: string; date: string; time: string; slots: Slot[]; slotsLoading: boolean; saving: boolean; price: number | string | null; onService: (value: string) => void; onEmployee: (value: string) => void; onDate: (value: string) => void; onTime: (value: string) => void; onCancel: () => void; onSubmit: (event: FormEvent) => void }) {
	return <section className="mx-auto max-w-3xl"><header className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">Appuntamento</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Modifica prenotazione</h1><p className="mt-2 text-sm leading-6 text-zinc-600">Le modifiche sono disponibili fino a 24 ore prima dell&apos;appuntamento.</p></div><button type="button" onClick={onCancel} aria-label="Torna alle prenotazioni" className="inline-flex size-10 shrink-0 items-center justify-center rounded-lg border border-zinc-200 bg-white text-zinc-700 shadow-sm transition hover:bg-zinc-100 hover:text-brand focus:outline-none focus:ring-2 focus:ring-brand/35"><ArrowLeft aria-hidden="true" className="size-5" /></button></header><form onSubmit={onSubmit} className="mt-7 grid gap-5 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:grid-cols-2 sm:p-7"><label className="text-sm font-medium text-zinc-800">Servizio<select value={serviceId} onChange={(event) => onService(event.target.value)} className="mt-2 min-h-11 w-full rounded-lg border border-zinc-300 bg-white px-3 text-zinc-900 outline-none transition focus:border-zinc-500 focus:ring-2 focus:ring-zinc-200">{catalog?.services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label><label className="text-sm font-medium text-zinc-800">Addetto<select value={employeeId} onChange={(event) => onEmployee(event.target.value)} className="mt-2 min-h-11 w-full rounded-lg border border-zinc-300 bg-white px-3 text-zinc-900 outline-none transition focus:border-zinc-500 focus:ring-2 focus:ring-zinc-200">{catalog?.employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select></label><label className="text-sm font-medium text-zinc-800">Giorno<input type="date" min={today()} max={maxDate()} value={date} onChange={(event) => onDate(event.target.value)} className="mt-2 min-h-11 w-full rounded-lg border border-zinc-300 bg-white px-3 text-zinc-900 outline-none transition focus:border-zinc-500 focus:ring-2 focus:ring-zinc-200" /></label><div className="sm:col-span-2"><p className="text-sm font-medium text-zinc-800">Orario</p>{slotsLoading ? <p className="mt-3 rounded-lg bg-zinc-50 p-3 text-sm text-zinc-600">Caricamento orari disponibili...</p> : <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-6">{slots.map((slot) => <button key={slot.time} type="button" disabled={slot.disabled} onClick={() => onTime(slot.time)} aria-pressed={time === slot.time} className={`min-h-11 rounded-lg border text-sm font-medium transition ${time === slot.time ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-300 bg-white text-zinc-800 hover:bg-zinc-50"} disabled:cursor-not-allowed disabled:border-zinc-200 disabled:bg-zinc-100 disabled:text-zinc-400`}>{slot.time}</button>)}</div>}</div><div className="rounded-xl bg-zinc-50 p-4 sm:col-span-2"><p className="text-xs font-medium uppercase tracking-wide text-zinc-500">Prezzo previsto</p><p className="mt-1 font-semibold text-zinc-900">{priceLabel(price)}</p></div><div className="flex flex-wrap justify-end gap-3 border-t border-zinc-200 pt-5 sm:col-span-2"><button type="button" onClick={onCancel} className="min-h-11 rounded-lg border border-zinc-300 bg-white px-4 text-sm font-medium text-zinc-800 transition hover:bg-zinc-50">Annulla</button><button disabled={!time || saving} className="min-h-11 rounded-lg bg-zinc-900 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:bg-zinc-300">{saving ? "Salvataggio..." : "Salva modifica"}</button></div></form></section>;
}
