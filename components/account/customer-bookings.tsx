...
function CompactDatePicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
	...
	return <Popover open={open} onOpenChange={setOpen}><PopoverTrigger asChild><button type="button" className="mt-2 flex min-h-11 w-52 items-center justify-between rounded-lg border border-zinc-300 bg-white px-3 text-left text-sm font-normal text-zinc-900 outline-none transition hover:bg-zinc-50 focus:border-zinc-500 focus:ring-2 focus:ring-zinc-200"><span className="capitalize">{compactDateFormatter.format(selected)}</span><CalendarDays className="size-4 text-zinc-500" /></button></PopoverTrigger><PopoverContent align="start" className="w-auto p-2"><Calendar mode="single" selected={selected} onSelect={(next) => { if (!next) return; onChange(toIsoDate(next)); setOpen(false); }} disabled={{ before: toLocalDate(today()), after: toLocalDate(maxDate()) }} /></PopoverContent></Popover>;
}
