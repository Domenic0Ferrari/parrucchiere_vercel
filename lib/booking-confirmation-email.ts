...
export async function sendBookingConfirmationEmail(booking: BookingEmail) {
	...
	const htmlContent = `<!doctype html><html lang="it"><body style="margin:0;background:#f7f7f7;font-family:Arial,sans-serif;color:#242827"><main style="max-width:560px;margin:32px auto;padding:28px;background:#fff;border:1px solid #e4e4e7;border-radius:16px"><h1 style="font-size:24px;margin:0 0 12px">Prenotazione confermata</h1><p>Ciao ${escapeHtml(booking.customerName)}, ti aspettiamo in salone!</p>${testNote}<table style="width:100%;border-collapse:collapse;margin:24px 0"><tbody><tr><td style="padding:10px 0;color:#666">Servizio</td><td style="padding:10px 0;text-align:right">${escapeHtml(booking.serviceName)}</td></tr><tr><td style="padding:10px 0;color:#666">Addetto</td><td style="padding:10px 0;text-align:right">${escapeHtml(booking.employeeName)}</td></tr><tr><td style="padding:10px 0;color:#666">Giorno</td><td style="padding:10px 0;text-align:right">${escapeHtml(date)}</td></tr><tr><td style="padding:10px 0;color:#666">Orario</td><td style="padding:10px 0;text-align:right">${escapeHtml(time)}</td></tr><tr><td style="padding:10px 0;color:#666">Durata</td><td style="padding:10px 0;text-align:right">${escapeHtml(duration)}</td></tr><tr><td style="padding:10px 0;color:#666">Prezzo</td><td style="padding:10px 0;text-align:right;font-weight:bold">${escapeHtml(price)}</td></tr></tbody></table><p style="font-size:13px;color:#666">Per modificare o annullare l’appuntamento, contatta direttamente il salone.</p></main></body></html>`;
